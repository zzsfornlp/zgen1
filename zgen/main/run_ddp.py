...
def main():
    ...
    devices = [int(z) for z in range(torch.cuda.device_count())]
    # assert len(devices)>=2, "Otherwise do not need to use DDP!"
    if len(devices) < 2:
        print(f"WARNING: get very few devices: {devices}", file=sys.stderr)
    world_size = len(devices)
    mp.spawn(main_run, args=(world_size, mname, args), nprocs=world_size, join=True)
    # --

# CUDA_VISIBLE_DEVICES=?? PYTHONPATH=?? python3 -m zgen.main.run_ddp zgen.main.train ...(args)
