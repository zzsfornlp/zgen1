...
__all__ = [
    "SVConf", "ScheduledValue", "ResultRecord", "TrainingProgressRecord", "MyStat",
]
...
from .seria import JsonSerializable, default_json_serializer
...
class SVConf(Conf):
    def __init__(self):
        ...
        self.which_idx = "uidx"  # count steps on which: aidx, eidx, iidx, uidx
...
class MyStat:
    def __init__(self):
        self.num_doc = 0
        self.count_tf = Counter()  # tok count
        self.count_idf = Counter()  # tok doc count
        self._idf = None  # tok -> idf
        self._ranks = None  # tok -> rank

    def invalid_caches(self):
        self._idf = None
        self._ranks = None

    def __repr__(self):
        return f"Stat[d={self.num_doc},t={len(self.count_tf)}]"

    def add_doc(self, tokens):
        self.invalid_caches()  # change things!!
        self.num_doc += 1
        for t in tokens:
            self.count_tf[t] += 1
        for t in set(tokens):
    ...
    def idf(self):
        if self._idf is None:
            # idf with smooth
            self._idf = {}
            for k, c in self.count_idf.items():
                _v = math.log((1 + self.num_doc) / (1 + c)) + 1
                self._idf[k] = _v
        return self._idf
    ...
    def ranks(self):
        if self._ranks is None:
            ranked_words = sorted(self.count_tf.keys(), key=(lambda x: self.count_tf[x]), reverse=True)
            self._ranks = {w: r for r, w in enumerate(ranked_words)}
        return self._ranks

    def get_idf(self, tok: str):
        _v = self.idf.get(tok)
        if _v is None:
            _v = math.log((1+self.num_doc)) + 1
        return _v

    def get_tfidf(self, tok: str, count: int, all_count: int):
        tf = (count / all_count)
        idf = self.get_idf(tok)
        return tf * idf

    def __iadd__(self, other):
        self.invalid_caches()
        self.num_doc += other.num_doc
        self.count_tf += other.count_tf
        self.count_idf += other.count_idf
        return self
    ...
    def create_from_dict(cls, d):
        ret = MyStat()
        ret.from_dict(d)
        zlog(f"Create Mystat: {ret}")
        return ret

    @classmethod
    def create_from_file(cls, f: str):
        d = default_json_serializer.from_file(f)
        return MyStat.create_from_dict(d)
